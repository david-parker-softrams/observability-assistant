...
class TestIntentNudgingFlow:
                ...
                tool_calls=[
                    {
                        ...
                        "function": {
                            ...
                            "arguments": json.dumps(
                                {"log_group": "/aws/lambda/test", "filter_pattern": "ERROR"}
                            ),
                        },
                    }
                ],
    ...
    async def test_list_intent_leads_to_list_call(self, e2e_settings, mock_sanitizer):
                ...
                tool_calls=[
                    {
                        "id": "call_1",
                        "type": "function",
                        "function": {"name": "list_log_groups", "arguments": json.dumps({})},
                    }
                ],
                finish_reason="tool_calls",
            ),
        ...
        await orchestrator.chat("Show log groups")
    ...
    async def test_multiple_intents_single_conversation(self, e2e_settings, mock_sanitizer):
                ...
                tool_calls=[
                    {
                        ...
                        "function": {
                            "name": "fetch_logs",
                            "arguments": json.dumps(
                                {"log_group": "/test", "filter_pattern": "ERROR"}
                            ),
                        },
                    }
                ],
        ...
        await orchestrator.chat("Check logs")
...
class TestIntentWithRetry:
    ...
    async def test_intent_after_empty_result(self, e2e_settings, mock_sanitizer):
        ...
        mock_llm.chat.side_effect = [
            ...
            LLMResponse(
                content="I'll expand the time range to find more logs.", finish_reason="stop"
            ),
                ...
                tool_calls=[
                    {
                        ...
                        "function": {
                            "name": "fetch_logs",
                            "arguments": json.dumps(
                                {"log_group": "/test", "start_time": "24h ago"}
                            ),
                        },
                    }
                ],
...
class TestEdgeCases:
    ...
    async def test_max_nudge_attempts(self, e2e_settings, mock_sanitizer):
        ...
        responses = []
        for _i in range(10):
            responses.append(LLMResponse(content="I'll search for logs.", finish_reason="stop"))
